 ...
 * models. Quality/cost/latency all come from the eval receipt
 * docs/model-eval-results.json (byModel.p50ms, full-turn wall clock).
 */
...
// [label, quality (proofloop 1-5), p50 full-turn latency s (receipt byModel.p50ms), $/turn, highlight]
const DATA = [
  ["gpt-5.4-mini", 4.75, 2.821, 0.00072, "default"],
  ["gpt-4.1-nano", 4.15, 1.999, 0.000033, "cheapest"],
  ["gpt-4.1-mini", 4.1, 1.959, 0.00014, ""],
  ["gpt-4o-mini", 4.5, 2.412, 0.000051, ""],
  ["gpt-5-nano", 4.6, 3.4, 0.00013, ""],
  ["gpt-5-mini", 5.0, 6.344, 0.00079, ""],
];

const W = 720, H = 440, M = { l: 64, r: 24, t: 56, b: 56 };
const iw = W - M.l - M.r, ih = H - M.t - M.b;
const xMin = 1.6, xMax = 6.8, yMin = 3.9, yMax = 5.1;
...
let els = "";
// grid + axes
for (let gx = 2; gx <= 6.5; gx += 0.5) els += `<line x1="${px(gx)}" y1="${M.t}" x2="${px(gx)}" y2="${M.t + ih}" stroke="${C.grid}" stroke-width="1"/><text x="${px(gx)}" y="${M.t + ih + 20}" fill="${C.mute}" font-size="11" text-anchor="middle">${gx}s</text>`;
...
els += `<rect x="${px(xMin)}" y="${py(yMax)}" width="${px(3.0) - px(xMin)}" height="${py(4.3) - py(yMax)}" fill="${C.success}" opacity="0.06"/>`;
els += `<text x="${px(2.0)}" y="${py(5.03)}" fill="${C.success}" font-size="11" font-weight="600" opacity="0.8">◱ fast + smart</text>`;
...
<text x="${M.l}" y="46" fill="${C.mute}" font-size="11">proofloop quality (judge, 1–5) · p50 full-turn latency (model-eval-results.json) · bubble ∝ $/turn</text>
${els}
<text x="${M.l + iw / 2}" y="${H - 12}" fill="${C.mute}" font-size="12" text-anchor="middle">p50 full-turn latency →</text>
